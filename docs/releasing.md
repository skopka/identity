...
all seven `Skopka.Identity` package IDs and store it as the environment secret
named `NUGET_API_KEY`.
...
git switch main
git pull --ff-only
git tag -a v0.9.0 -m "Skopka.Identity 0.9.0"
...
test suite, audits dependencies, verifies the exact seven packages and symbol
...
for every package ID. It then pushes all seven packages in dependency order without
