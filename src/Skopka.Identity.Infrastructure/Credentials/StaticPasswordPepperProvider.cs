...
public sealed class StaticPasswordPepperProvider : IPasswordPepperProvider, IDisposable
{
    private readonly IReadOnlyDictionary<string, byte[]> keys;
    private bool disposed;

    public StaticPasswordPepperProvider(string currentKeyId, byte[] currentKey)
        : this(
            currentKeyId,
    ...
    public StaticPasswordPepperProvider(
        string currentKeyId,
        IReadOnlyDictionary<string, byte[]> keys)
    {
        ...
        if (!PasswordVerifierCodec.IsValidKeyId(currentKeyId))
        {
            throw new ArgumentException(
                "Pepper key id must contain only ASCII letters, digits, '.', '_' or '-'.",
                nameof(currentKeyId));
        }

        foreach (var (keyId, key) in keys)
        {
            if (!PasswordVerifierCodec.IsValidKeyId(keyId))
            {
                throw new ArgumentException(
                    $"Pepper key id '{keyId}' contains unsupported characters.",
                    nameof(keys));
            }
            ...
            {
                throw new ArgumentException(
                    "Each password pepper key must contain at least 32 bytes.",
                    nameof(keys));
            }
        }
        ...
        if (!copiedKeys.ContainsKey(currentKeyId))
        {
            ...
            throw new ArgumentException(
                "The current password pepper key id is not present in the key collection.",
                nameof(currentKeyId));
        }
